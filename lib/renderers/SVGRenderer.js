/**
 * @modified Fri 30 Dec 2022 Isaiah Odhner:
 * 1. Added support for styling Sprite objects with adhoc SpriteMaterial.styleForSVGRenderer,
 *    to support texturing sprites using an SVG pattern fill.
 * 2. Added escape hatch preventRemoval flag check on DOM nodes when clearing SVG,
 *    to support a persistent <defs> object for defining pattern fills.
 * 3. When pieces with the same image pattern (pawns in particular, with the
 *    standard board layout) got sorted contiguously in z-order, the pattern
 *    actually SPANNED the different pieces, because it was generating a single
 *    SVG <path> element for them, as an optimization. I added a hacky check for
 *    the string "pattern" in the style, to prevent merging these paths in my use case.
 * 4. Added support for SpriteMaterial.rotation
 *    (Sprite.center is not respected.)
 */
...
class SVGRenderer {
	...
	constructor() {
		...
		function removeChildNodes() {
			...
			for ( let i = _svg.childNodes.length - 1; i >= 0; i -= 1 ) {

				if ( ! _svg.childNodes[ i ].preventRemoval ) {
		...
		function renderSprite( v1, element, material ) {
			...
			if ( material.rotation ) {
				...
				style += ';transform:rotate(' + convert( material.rotation ) + 'rad);transform-origin:' + convert( v1.x ) + 'px ' + convert( v1.y ) + 'px';

			}

			if ( material.styleForSVGRenderer ) {
				
				style += ';' + material.styleForSVGRenderer;
		...
		function addPath( style, path ) {
			...
			if ( _currentStyle === style && !style.includes("pattern")) {
